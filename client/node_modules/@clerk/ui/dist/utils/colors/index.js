...
import { colors as colors$1 } from "./legacy.js";
import { colors as colors$2 } from "./modern.js";
...
const colors = {
	...
	* ```
	*/
	changeHslaLightness: colors$1.changeHslaLightness,
	/**
	...
	* ```
	*/
	setHslaAlpha: colors$1.setHslaAlpha,
	/**
	...
	*/
	toHslaColor: (color) => {
		if (!color) return;
		return cssSupports.modernColor() ? color : colors$1.toHslaColor(color);
	},
	/**
	...
	*/
	toHslaString: (color) => {
		...
		return colors$1.toHslaString(color);
	},
	/**
	...
	*/
	lighten: (color, percentage = 0) => {
		if (cssSupports.modernColor()) return colors$2.lighten(color, percentage);
		return colors$1.lighten(color, percentage);
	},
	/**
	...
	*/
	makeTransparent: (color, percentage = 0) => {
		if (cssSupports.modernColor()) return colors$2.makeTransparent(color, percentage);
		return colors$1.makeTransparent(color, percentage);
	},
	/**
	...
	*/
	makeSolid: (color) => {
		if (cssSupports.modernColor()) return colors$2.makeSolid(color);
		return colors$1.makeSolid(color);
	},
	/**
	...
	*/
	setAlpha: (color, alpha) => {
		if (cssSupports.modernColor()) return colors$2.setAlpha(color, alpha);
		return colors$1.setAlpha(color, alpha);
	},
	/**
	...
	*/
	adjustForLightness: (color, lightness = 5) => {
		if (cssSupports.modernColor()) return colors$2.adjustForLightness(color, lightness);
		return colors$1.adjustForLightness(color, lightness);
	}
};
